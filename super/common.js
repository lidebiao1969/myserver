...
common.post("/process_post", function(req, res) { //urlencodedParser,
    // console.log(req);
    // 输出 JSON 格式
    var response = { //
...
common.post("/file_upload", function(req, res) {
    ...
    var des_file = process.cwd() + "/static/image/" + req.files[0].originalname;
...
common.get("/listUsers", function(req, res) {
    fs.readFile(process.cwd() + "/lunbo.json", "utf8", function(err, data) {
        // console.log(data);
        res.end(data);
    });
});
common.get("/aq_lunbo", function(req, res) {
    fs.readFile(process.cwd() + "/anquan/img-lunbo.json", "utf8", function(err, data) {
        // console.log(data);
